...
function useUpdateSpaceCardPosition(
  ...
  const { socketEmitWithAuth, socket } = useSocket(spaceCardRef.current.targetSpaceId);

  function handleUpdatePosition(data: SpaceCardImmediateUpdatePositionRequestDTO) {
