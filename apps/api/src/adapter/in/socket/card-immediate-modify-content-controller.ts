...
const CardImmediateModifyContentController: IoControllerInterface<CardImmediateModifyContentUseCase> = (
  socket,
  io,
  cardImmediateModifyContent
) => {
  socket.on(
    'card:modify-content',
    async (
      data: CardContentModifyEventDTO & Authorization) => {
      try {
        const accountToken = getAccountTokenInterfaceFromAuth(data);
        const card = await cardImmediateModifyContent({
          accountId: accountToken?.accountId,
          ...data
        });
        io.emit('card:modified-content', card);
      } catch (error) {}
    }
  );
